    ...
    const menuItems = [
        {
            ...
            onClick: () => { navigate("/doctor/"); setOpenDrawer(false); },
        },
        {
            key: "appointments",
            icon: <FaListCheck size={20} />,
            label: "Xem lịch hẹn",
            onClick: () => { navigate("/doctor/appointments"); setOpenDrawer(false); },
        },
        {
            key: "treatments",
            icon: <FaHandHoldingMedical size={20} />,
            label: "Khám bệnh",
            onClick: () => { navigate("/doctor/treatments"); setOpenDrawer(false); },
        },
        ...
        <Layout className="h-screen overflow-hidden">
            ...
            <Layout className="flex flex-col">
                ...
                <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t flex justify-around py-2 z-50 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
                    <div className="flex flex-col items-center text-blue-600" onClick={() => navigate("/doctor")}>
                        <CgProfile size={22} />
                        <span className="text-[10px] mt-1">Cá nhân</span>
                    </div>
                    <div className="flex flex-col items-center text-gray-500" onClick={() => navigate("/doctor/appointments")}>
                        <FaListCheck size={22} />
                        <span className="text-[10px] mt-1">Lịch hẹn</span>
                    </div>
                    <div className="flex flex-col items-center text-gray-500" onClick={() => navigate("/doctor/treatments")}>
                        <FaHandHoldingMedical size={22} />
                        <span className="text-[10px] mt-1">Khám bệnh</span>
                    </div>
                </div>
            </Layout>
        </Layout>
